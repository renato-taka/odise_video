...
class LatentDiffusion(nn.Module):
    ...
    def __init__(
        ...
        #为ldm.cond_stage_model类添加device属性
    ...
    def forward_test(self, batched_inputs):
        ...
        if self.guidance_scale != 1.0:
            ...
            #如果guidance_scale不等于1.0，那么就将uncond_inputs扩展成和cond_inputs一样的形状
        else:
            uncond_inputs = None
        ...
        if uncond_inputs is None:
            latent_samples = self.diffusion.ddim_sample_loop(
                ...
                shape=(batch_size, self.latent_dim, *self.latent_image_size),#元组解包为独立值
                device=self.device,
                clip_denoised=False,
...
class LdmExtractor(FeatureExtractor):
    ...
    def forward(self, batched_inputs):
        ...
        return features
    #输出为一个list，包含了各个层的输出，每个输出的shape为(batch_size, channel, height, width)
...
class LdmImplicitCaptionerExtractor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        #获取文本嵌入形状，并将其保存在text_embed_shape中
